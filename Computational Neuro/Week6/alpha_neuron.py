...
def runthisbitch(t_peak): 

    ref = 0 # refractory period counter

    spiketimes = [] #Record Spike Times
...
for x in range(1,10,1):
    print("T_peak = {}".format(x))
    spikecount.append(runthisbitch(x))
    print("Number of spikes = {}".format(spikecount[len(spikecount)-1]))
    
z = np.asarray(range(1,10,1))
w = np.asarray(spikecount)
